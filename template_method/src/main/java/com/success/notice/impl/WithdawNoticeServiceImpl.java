 ...
 */
@Service
public class WithdawNoticeServiceImpl extends AbstractNoticeService {


    @Override
    protected void updateNoticeStatus(NoticeResult noticeResult, NoticeMessage noticeMessage) {
        System.out.println(">>>>>WithdawNoticeServiceImpl 执行通知状态更新,消息信息:"+ JSON.toJSONString(noticeMessage)+",通知结果为:"+JSON.toJSONString(noticeResult));
    }

    @Override
    protected String buildNoticeContent(String uniqueNo) {
        System.out.println(">>>>>WithdawNoticeServiceImpl 构建通知内容,uniqueNo:"+ uniqueNo);
        JSONObject data = new JSONObject();
        data.put("serviceName","WithdawNoticeService");
